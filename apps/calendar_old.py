...
def prep_columns_for_calendar_view(df):
    for i in range(3, 10):
...
def get_year(dataframe, year):
    return prep_columns_for_calendar_view(dataframe[dataframe["year"] == year])
...
layout = DataTable(
    data=year.to_dict('records'),
    columns=[{'id': c, 'name': c} for c in year.columns][3:],
    page_size=5,
    page_current=0,
    virtualization=True,
